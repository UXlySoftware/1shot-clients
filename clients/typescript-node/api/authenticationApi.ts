...
import { GetAccessToken200Response } from '../model/getAccessToken200Response';
...
export class AuthenticationApi {
     ...
     * @param clientId A valid API Key value from a defined API Credential
     * @param clientSecret A valid Secret value from a defined API Credential
     */
    public async getAccessToken (grantType?: string, clientId?: string, clientSecret?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: GetAccessToken200Response;  }> {
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: GetAccessToken200Response;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "GetAccessToken200Response");
