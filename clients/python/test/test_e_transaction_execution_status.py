# coding: utf-8

"""
    M2M Gateway API

    The M2M Gateway API is for communication by 3rd party servers for automated tasks in the Framework

    The version of the OpenAPI document: 0.1
    Contact: support@uxly.software
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.e_transaction_execution_status import ETransactionExecutionStatus

class TestETransactionExecutionStatus(unittest.TestCase):
    """ETransactionExecutionStatus unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def testETransactionExecutionStatus(self):
        """Test ETransactionExecutionStatus"""
        # inst = ETransactionExecutionStatus()

if __name__ == '__main__':
    unittest.main()
